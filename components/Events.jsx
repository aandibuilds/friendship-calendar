...
export default function Events({ friends, events, proposed, onUpdateAll }) {
  ...
  return (
    <div className="view">
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {activeProposed.map(ev => {
              ...
              const countdownText = daysUntil === null ? '' : daysUntil === 0 ? 'Today' : daysUntil === 1 ? 'Tomorrow' : `${daysUntil}d away`;
                ...
                <div key={ev.id} className="propose-card">
                  <div style={{ paddingLeft: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      <button type="button" className="btn-copy" onClick={() => { navigator.clipboard.writeText(shareLink); showToast('Link copied.'); }}>Copy invite link</button>
      ...
      {/* EVENTS LIST */}
      {events.length === 0 ? (
        <div className="empty-state"><div className="empty-title">No events yet</div><div className="empty-sub">Create an event to start planning.</div></div>
      ) : (
        events.map(ev => {
          ...
          return (
            <div key={ev.id} className="event-card">
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, marginBottom: 8 }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--plum-pale)', border: '1px solid rgba(124, 58, 237, 0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>{ev.emoji || '•'}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                    <div className="event-name">{ev.name}</div>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 99, background: isGroup ? 'rgba(91, 79, 255, 0.12)' : 'var(--health-strong-bg)', color: isGroup ? 'var(--primary)' : 'var(--health-strong)', fontWeight: 600 }}>{isGroup ? 'Group' : '1-on-1'}</span>
                  </div>
              ...
              {(ev.invitees || []).length > 0 && (
                <div style={{ display: 'flex', gap: 5, marginTop: 9, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {(ev.invitees || []).map(id => { const f = friends.find(x => x.id === id); return f ? <div key={id} style={{ width: 24, height: 24, borderRadius: '50%', background: f.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display), Georgia, serif', fontSize: 10, fontWeight: 600, color: 'white' }} title={f.name}>{f.name[0]}</div> : null; })}
              ...
              {isGroup && (
                <>
                  <div className="rsvp-row">
                    <div className="rsvp-chip">Yes {yes}</div>
                    <div className="rsvp-chip">Maybe {maybe}</div>
                    <div className="rsvp-chip">No {no}</div>
                  </div>
