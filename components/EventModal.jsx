    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-header" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: 18, fontWeight: 600, letterSpacing: '-0.02em' }}>
              {step === 1 ? 'Create an event' : step === 2 ? (type === 'group' ? '🎉 Group event details' : '🤝 1-on-1 hangout details') : 'Invite your friends'}
            </div>
        ...
        <div style={{ height: 3, background: 'var(--border)' }}>
          <div style={{ height: 3, background: 'var(--terra)', width: `${progress}%`, transition: 'width 0.3s', borderRadius: '0 99px 99px 0' }} />
        </div>
        ...
        <div className="modal-body">
          ...
          {step === 1 && (
            <>
              <p style={{ fontSize: '13.5px', color: 'var(--ink-muted)', marginBottom: 20 }}>What kind of event is this?</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[['group', '🎉', 'Group hangout', 'Invite multiple friends, run a poll for the best date, collect RSVPs'],
                  ['1on1',  '🤝', '1-on-1 hangout', 'Plan time with one specific friend and log it to their profile']
          ...
          {step === 2 && type === 'group' && (
            <>
              ...
              <div className="form-group">
                <label className="form-label">Emoji / vibe</label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7, maxHeight: 220, overflowY: 'auto' }}>
                  {friends.map(f => (
                    <div key={f.id} className={`solo-friend-row ${soloFriend === f.id ? 'selected' : ''}`} onClick={() => setSoloFriend(f.id)}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: f.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Fraunces, serif', fontSize: 13, fontWeight: 600, color: 'white', flexShrink: 0 }}>{f.name[0]}</div>
                      ...
                      <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${soloFriend === f.id ? 'var(--terra)' : 'var(--border)'}`, background: soloFriend === f.id ? 'var(--terra)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 10, fontWeight: 700 }}>{soloFriend === f.id ? '✓' : ''}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {step === 3 && (
            <>
              ...
              {invitees.size > 0 && (
                <div style={{ marginTop: 14, padding: '12px', background: 'var(--parchment)', borderRadius: 'var(--radius-xs)', fontSize: 13, color: 'var(--ink-muted)' }}>
