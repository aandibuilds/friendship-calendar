/*
  Resend email helper — sends custom emails.
...
export async function sendPasswordResetEmail({ to, resetLink }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM_EMAIL || 'Friendship Calendar <onboarding@resend.dev>',
      to,
      subject: 'Reset your Friendship Calendar password',
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 24px;">
          <div style="text-align: center; margin-bottom: 32px;">
            <div style="display: inline-block; width: 56px; height: 56px; border-radius: 14px; background: linear-gradient(135deg, #7C3AED, #B07ACC); color: white; font-size: 20px; font-weight: 700; line-height: 56px; letter-spacing: -1px;">FC</div>
          </div>
          <h1 style="font-size: 22px; font-weight: 700; color: #1a1a2e; margin: 0 0 8px; text-align: center;">Reset your password</h1>
          <p style="font-size: 15px; color: #4B5563; text-align: center; margin: 0 0 28px;">
            We received a request to reset your Friendship Calendar password. Click the button below to choose a new one.
          </p>
          <div style="text-align: center; margin-bottom: 28px;">
            <a href="${resetLink}" style="display: inline-block; background: linear-gradient(135deg, #7C3AED, #9F67E4); color: white; text-decoration: none; padding: 14px 32px; border-radius: 12px; font-size: 16px; font-weight: 700;">
              Reset Password
            </a>
          </div>
          <p style="font-size: 12px; color: #9CA3AF; text-align: center;">
