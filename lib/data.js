...
export function loadData() {
  if (typeof window === 'undefined') {
    return { friends: [], events: [], proposed: [], quiet: defaultQuiet(), profile: defaultProfile() };
  }
  try {
    return {
      friends:  safeJsonParse(localStorage.getItem('fc2_friends'), []),
      events:   safeJsonParse(localStorage.getItem('fc2_events'), []),
      proposed: safeJsonParse(localStorage.getItem('fc2_proposed'), []),
      quiet:    safeJsonParse(localStorage.getItem('fc2_quiet'), defaultQuiet()),
      profile:  safeJsonParse(localStorage.getItem('fc2_profile'), defaultProfile()),
    };
  } catch {
    return { friends: [], events: [], proposed: [], quiet: defaultQuiet(), profile: defaultProfile() };
  }
...
export function saveData({ friends, events, proposed, quiet, profile }) {
  ...
  try {
    ...
    /* quota / private mode / blocked storage */
  }
}

function defaultQuiet() { return { enabled: false, start: '22:00', end: '08:00' }; }
function defaultProfile() {
  return { name: '', color: '#5B4FFF', location: '', birthday: '', bio: '', hobbies: [], times: [], hangtypes: [], social: '', vibe: '' };
}
...
export function initSample(friends, events) {
  ...
  const sampleFriends = [
    { id: 1, name: 'Maya Patel',      color: '#5B4FFF', tags: ['College'],          cadence: 30, notes: 'Loves Thai food. Cat named Miso. Big fan of Studio Ghibli films. Usually free on weekends.', hangouts: [{ date: ago(65), activity: 'Coffee at Blue Bottle' }, { date: ago(120), activity: 'Hiking' }, { date: ago(180), activity: 'Dinner at Rintaro' }, { date: ago(35), activity: 'Movie night' }] },
    { id: 2, name: 'James Liu',       color: '#14B8A6', tags: ['Work', 'College'],  cadence: 30, notes: 'Into photography. Matcha obsessed. Goes to the farmers market every Sunday. Loves ramen.', hangouts: [{ date: ago(10), activity: 'Ramen lunch' }, { date: ago(42), activity: 'Basketball' }, { date: ago(73), activity: 'Gallery opening' }, { date: ago(104), activity: 'Bike ride' }] },
    { id: 3, name: 'Rosa Fernández',  color: '#D946EF', tags: ['Neighbors'],        cadence: 14, notes: 'Next door neighbor. Incredible cook, especially Mexican food. Loves gardening and yoga.', hangouts: [{ date: ago(20), activity: 'Dinner at her place' }, { date: ago(50), activity: 'Farmers market' }, { date: ago(68), activity: 'Coffee' }] },
    { id: 4, name: 'Tom Brennan',     color: '#7C3AED', tags: ['Childhood'],        cadence: 90, notes: 'Back in Boston. Celtics fan. Works in finance. Loves craft beer and hiking.', hangouts: [{ date: ago(110), activity: 'Video call' }, { date: ago(200), activity: 'Visited Boston' }] },
    { id: 5, name: 'Keiko Tanaka',    color: '#FB7185', tags: ['Work'],             cadence: 30, notes: 'Met at conference. Loves ceramics and pottery. Always up for trying new restaurants.', hangouts: [{ date: ago(5), activity: 'Coffee catch-up' }, { date: ago(37), activity: 'Museum visit' }, { date: ago(68), activity: 'Lunch' }] },
  ];
  const sampleEvents = [{
...
export function getHealthColor(s) { return s >= 70 ? 'var(--health-strong)' : s >= 40 ? 'var(--health-mid)' : 'var(--health-low)'; }
